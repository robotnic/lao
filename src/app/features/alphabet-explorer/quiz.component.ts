      ...
      @if (currentQuestion(); as question) {
        <div class="quiz-content">
          <div class="character-section">
            ...
            <div class="character-name">{{ question.correctName }}</div>
          </div>

          <div class="question-section">
            <p class="question">What does this character mean?</p>
            ...
            @if (question.answered && question.isCorrect === false) {
              <div class="feedback incorrect">
                ✗ Incorrect. Mnemonic: {{ question.correctMnemonic }}
              </div>
            }
...
export class AlphabetQuizComponent implements OnInit {
  ...
  readonly title = 'Alphabet Mastery Quiz';
  ...
  private initializeQuiz(): void {
    const alphabet = this.jsonDataProvider.alphabet();
    if (!alphabet || alphabet.length === 0) {
      console.error('No alphabet data available');
      return;
    }

    // Get characters with learning/review/mastered status (at least 5 for quiz)
    const masteredChars = alphabet.filter(char => {
      const item = this.progressService.items().find(i => i.id === char.id);
      return item && ['learning', 'review', 'mastered'].includes(item.srsState);
    });

    // If not enough mastered chars, use all chars for learning
    const quizChars = masteredChars.length >= 5 ? masteredChars : alphabet;

    // Shuffle and take up to 10 questions
    const shuffled = this.shuffleArray([...quizChars]).slice(0, 10);

    const questions = shuffled.map(char => this.createQuestion(char as any));
    this.questions.set(questions);
  }

  private createQuestion(char: any): QuizQuestion {
    const alphabet = this.jsonDataProvider.alphabet();
    const otherChars = alphabet.filter(c => c.id !== char.id);

    // Create 3 plausible distractors
    const distractors = this.shuffleArray(otherChars)
      .slice(0, 3)
      .map(c => (c as any).mnemonic || (c as any).name);

    const options = this.shuffleArray([
      char.mnemonic || char.name,
      ...distractors,
    ]);
    ...
    return {
      ...
      correctMnemonic: char.mnemonic || char.name,
      options,
      selectedIndex: null,
      isCorrect: null,
      answered: false,
  ...
  finishQuiz(): void {
    ...
    this.router.navigate(['/alphabet-explorer']);
  }
  ...
  correctIndex(question: QuizQuestion): number {
    return question.options.indexOf(question.correctMnemonic);
  }
  ...
  calculateLevel(): string {
    ...
    return 'Review the characters';
  }

  private saveProgress(): void {
    // Update progress for each correctly answered character
    this.questions().forEach(q => {
      if (q.isCorrect) {
        this.progressService.updateItemProgress(q.characterId, 'character', true);
      }
    });
  }
